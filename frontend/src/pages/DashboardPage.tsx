    ...
    >
      ...
      <div className="db-task__body">
        <div className="db-task__head">
          <div className="db-task__title-row">
...
const DashboardPage: React.FC = () => {
  ...
  const fetchTasks = useCallback(async () => {
    try {
      setLoading(true);
      setError('');
      const status = statusFilter !== 'All' ? (statusFilter === 'In Progress' ? 'In-Progress' : statusFilter) : undefined;
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const payload: CreateTaskInput = { title, description, priority, due_date: dueDate || null, status: 'Todo' };
  ...
  const updateStatus = async (id: string, status: string) => {
    ...
    } catch (err: any) { setError(err?.response?.data?.error || 'Failed to update task.'); }
  };
    ...
    <div className="db">
      ...
      <div className="db__header">
        <div>
          ...
          <p className="db__subtitle">
            {stats.total === 0 ? 'No tasks yet — create one to get started' : `${stats.done} of ${stats.total} tasks completed · ${completionPct}% done`}
          </p>
        </div>
      ...
      <div className="db__filters">
        <div className="db__filter-tabs">
          {FILTERS.map(f => (
            <button key={f} className={`db__filter-tab${statusFilter === f ? ' db__filter-tab--active' : ''}`} onClick={() => setStatusFilter(f)}>
              {f}
              {f !== 'All' && (
