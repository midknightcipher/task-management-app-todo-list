...
# NEW: Logs the pipeline run so the React Frontend widget turns green!
def log_pipeline_run(status, rows_processed, error_message=None):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute("""
            INSERT INTO etl_pipeline_logs (run_start, run_end, status, rows_processed, error_message)
            VALUES (NOW() - INTERVAL '2 seconds', NOW(), %s, %s, %s)
...
def run_personal_metrics_etl():
    ...
    try:
        # FIX: Calculate actual overdue tasks using due_date instead of invalid status
        query = """
        ...
        transformed_data = [(r['user_id'], r['metric_date'], r['tasks_completed'], r['tasks_created'], r['overdue_tasks'], 
                            (r['tasks_completed']/r['tasks_created']*100 if r['tasks_created'] > 0 else 0), 85, 2.5) for r in rows]

        upsert_query = """
            INSERT INTO user_daily_metrics (user_id, metric_date, tasks_completed, tasks_created, overdue_tasks, completion_rate, productivity_score, avg_completion_time_hrs)
    ...
    try:
        ...
        for r in rows:
            ...
            transformed_data.append((
                ...
                r['overdue_tasks'], r['active_members'], overdue_pct, health_score
            ))

        upsert_query = """
            INSERT INTO workspace_daily_metrics (workspace_id, metric_date, completed_tasks, total_tasks, overdue_tasks, active_members, overdue_percentage, workspace_health_score)
            ...
            overdue_percentage = EXCLUDED.overdue_percentage, workspace_health_score = EXCLUDED.workspace_health_score, updated_at = NOW();
        """
        psycopg2.extras.execute_values(cursor, upsert_query, transformed_data)
        conn.commit()
...
def run_all_pipelines():
    try:
        ...
        # Log successful execution to trigger the frontend UI!
        log_pipeline_run('success', p_rows + w_rows)
        logging.info(f"Pipeline cycle complete. Processed {p_rows + w_rows} metrics rows.")
    except Exception as e:
        log_pipeline_run('failed', 0, str(e))
